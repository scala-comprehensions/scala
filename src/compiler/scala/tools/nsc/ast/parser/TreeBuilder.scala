  ...
  */
  private def makeFor(mapName: TermName, flatMapName: TermName, enums: List[Enumerator], body: Tree): Tree = {
    ...
    def makeForTransformer(t: ForTransformer, before: List[Enumerator]): Tree = {
      ...
      def generatedBody = {
        val tuple = makeTupleTerm(alreadyBoundNames.map(Ident.apply), flattenUnary = true)
        val selectionNames = List[TermName]("scala", "util", "internal", "Identity")
        val selection = selectionNames.foldLeft[Tree](Ident(nme.ROOTPKG))(Select.apply)
        Apply(selection, List(tuple))
      }
